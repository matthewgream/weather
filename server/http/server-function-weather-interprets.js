...
function interpretPressure(results, situation, data, data_history, store) {
    ...
    let min24h = adjustedPressure, max24h = adjustedPressure;
    ...
    if (month >= 9 || month <= 3) { // Fall through early spring
        ...
        if (adjustedPressure < 1005 && humidity > 70) 
            results.phenomena.push('summer low pressure - showers possible');
         else if (adjustedPressure > 1020) 
            results.phenomena.push('summer high pressure - warm and dry');
    }
    ...
    if (adjustedPressure < 1000 && Math.abs(change3h) > 3) 
        results.phenomena.push('barometric pressure changes may affect sensitive individuals');
...
function interpretTemperature(results, situation, data, data_history, store) {
    ...
    let min24h = temp, max24h = temp, min7d = temp, max7d = temp;
    let minTime24h = timestamp, maxTime24h = timestamp;
    Object.entries(data_history)
    ...
    const change1h = temp1hAgo !== undefined ? temp - temp1hAgo : 0,
     change6h = temp6hAgo !== undefined ? temp - temp6hAgo : 0;
    ...
    if (month >= 11 || month <= 2) { // Winter
        ...
        if (hour >= 0 && hour <= 6 && temp > temp6hAgo + 3) 
        ...
        if (hour >= 0 && hour <= 6 && min24h > 20) 
            results.phenomena.push('tropical night (min temp > 20°C)');
    }

    if (temp <= 0 && temp24hAgo > 0) {
        store.temperature.frostDays++;
        results.phenomena.push('first frost of the period');
    } else if (temp > 0 && min24h <= 0) 
        results.phenomena.push('frost occurred in last 24 hours');

    if (max7d - min7d > 30) 
        results.phenomena.push(`extreme weekly temperature range: ${(max7d - min7d).toFixed(1)}°C`);

    const minHour = new Date(minTime24h).getHours(),
    maxHour = new Date(maxTime24h).getHours();
    if (minHour >= 10 && minHour <= 18) 
        results.phenomena.push('unusual daytime temperature minimum');
    if (maxHour >= 22 || maxHour <= 6) 
        results.phenomena.push('unusual nighttime temperature maximum');

    if (location.elevation > 500 && temp < temp24hAgo - 5) 
...
function interpretHumidity(results, situation, data, data_history, store) {
    ...
    const sixHoursAgo = timestamp - 6 * 60 * 60 * 1000,
     twentyFourHoursAgo = timestamp - 24 * 60 * 60 * 1000;
    let humidity6hAgo;
    let min24h = humidity, max24h = humidity;
    let daytimeSum = 0, daytimeCount = 0;
    let nighttimeSum = 0, nighttimeCount = 0;
    ...
    const change6h = humidity6hAgo !== undefined ? humidity - humidity6hAgo : 0;
    if (Math.abs(change6h) > 30) 
        results.phenomena.push(`rapid humidity ${change6h > 0 ? 'increase' : 'decrease'}: ${Math.abs(change6h)}% in 6 hours`);

    if (situation.dewPoint !== undefined) {
        const dewPointSpread = temp - situation.dewPoint;
    ...
    if (month >= 11 || month <= 2) { // Winter
        if (humidity < 40 && temp < -10) 
            results.phenomena.push('arctic dry air');
        ...
        if (humidity > 80 && temp > 20) {
            ...
            if (store.humidity.consecutiveDryHours > 72) 
                results.alerts.push('prolonged dry conditions - fire risk');
        }
    }
    ...
    if (diurnalRange > 40) {
        ...
        if (store.humidity.avgDaytime < store.humidity.avgNighttime - 20) 
            results.phenomena.push('typical daily humidity cycle');
    } else if (diurnalRange < 10) 
        results.phenomena.push('stable humidity levels');
    ...
    if (location.forestCoverage === 'high') {
        ...
        if (humidity < 30 && windSpeed > 10) 
            results.phenomena.push('desiccating conditions in forest');
    }
    ...
    if (humidity < 30 && store.humidity.consecutiveDryHours > 24) {
        ...
        const apparentTemp = situation.heatIndex || temp;
        if (apparentTemp > temp + 5) 
            results.phenomena.push('humidity making it feel much warmer');
    }

    if (rainRate > 0 && humidity < 70) 
        results.phenomena.push('dry air - limited precipitation despite rain');
     else if (humidity > 95 && rainRate === 0) 
        results.phenomena.push('saturated air - precipitation imminent');

    if (hour >= 18 || hour <= 6) {
        if (humidity < 30 && temp < 0) 
            results.phenomena.push('indoor heating will create very dry conditions');
         else if (humidity > 80 && temp > 0) 
...
function interpretWindSpeed(results, situation, data, data_history, store) {
    ...
    const sixHoursAgo = timestamp - 6 * 60 * 60 * 1000,
     twentyFourHoursAgo = timestamp - 24 * 60 * 60 * 1000;
    let maxSpeed24h = windSpeed, maxGust24h = windGust || windSpeed;
    let speedSum6h = 0, speedCount6h = 0;
    const directionHistory = [];
    let calmHours = 0, stormHours = 0;
    Object.entries(data_history)
    ...
    if (windSpeed < 0.5) {
        ...
        if (location.forestCoverage === 'high') 
            results.phenomena.push('trees swaying noticeably');
    } else if (windSpeed < 13.8) {
        results.conditions.push('strong breeze');
        if (location.forestCoverage === 'high') 
            results.phenomena.push('large branches in motion');
        ...
        if (location.forestCoverage === 'high') 
            results.alerts.push('risk of falling branches');
        ...
        if (location.forestCoverage === 'high') 
            results.alerts.push('severe tree damage likely');
    ...
    if (store.wind.gustFactor > 1.7) {
        ...
        if (windSpeed > 10) 
            results.alerts.push('dangerous gusts');
    } else if (store.wind.gustFactor > 1.4) 
    ...
    if (windDir !== undefined && directionHistory.length > 5) {
        ...
        if (month >= 11 || month <= 2) { // Winter
            if (cardinalDir === 'N' || cardinalDir === 'NE') {
                ...
                results.phenomena.push('milder Atlantic influence');
        }
    }

    if (month >= 9 && month <= 3) { // Fall through spring
        if (windSpeed > 15) {
    ...
    if (windVariability > 10) 
        results.phenomena.push('highly variable wind speeds');
     else if (windVariability < 2 && windSpeed > 5) 
        results.phenomena.push('steady wind conditions');
    ...
    if (location.forestCoverage === 'high') {
        if (windSpeed > 5 && windSpeed < 10) 
            ...
            if (rainRate > 0 || data.snowDepth > 0) 
                results.alerts.push('increased risk of tree damage');
        }
    }

    if (temp < 10 && windSpeed > 3) {
        const windChillEffect = situation.windChill - temp;
        if (windChillEffect < -10) 
            results.alerts.push(`severe wind chill: feels like ${situation.windChill.toFixed(0)}°C`);
         else if (windChillEffect < -5) 
            results.phenomena.push(`significant wind chill effect`);
    }
    ...
    if (store.wind.stormHours > 6) 
        results.alerts.push(`prolonged storm: ${store.wind.stormHours} hours of gale-force winds`);

    if (hour >= 10 && hour <= 16 && windSpeed > store.wind.avgSpeed6h * 1.5) 
        results.phenomena.push('daytime wind strengthening');
     else if ((hour >= 22 || hour <= 4) && windSpeed < store.wind.avgSpeed6h * 0.5) 
        results.phenomena.push('nocturnal wind calming');

    if (location.elevation > 500) {
        if (windSpeed > 10) 
            results.phenomena.push('exposed elevation - enhanced wind');
        if (hour >= 18 || hour <= 6) 
            if (windSpeed < 2) 
...
function interpretCloudCover(results, situation, data, data_history, store) {
    ...
    let cloudCover6hAgo;
    let daytimeSum = 0, daytimeCount = 0,
     nighttimeSum = 0, nighttimeCount = 0;
    let clearHours = 0, overcastHours = 0,
     sunshineHours = 0;
    const cloudChanges = [];
    ...
    Object.entries(data_history)
        ...
        .forEach(([timestamp, entry]) => {
            ...
            const entryDate = new Date(timestamp),
             ...
             entryDaylight = getDaylightHours(situation.location.latitude, situation.location.longitude, entryDate);
            if (entryDaylight.isDaytime) {
                daytimeSum += entry.cloudCover;
                daytimeCount++;
                if (entry.cloudCover < 30 && entry.solarRad > 200) 
                    sunshineHours++;
            } else {
    ...
    if (cloudCover < 10) {
        ...
        if (daylight.isDaytime) 
            results.phenomena.push('full sunshine');
         else if (situation.location.lightPollution === 'low') 
            results.phenomena.push('excellent stargazing conditions');
    } else if (cloudCover < 30) {
        results.conditions.push('mostly clear');
        if (daylight.isDaytime) 
            results.phenomena.push('mostly sunny');
    } else if (cloudCover < 70) {
        results.conditions.push('partly cloudy');
        if (daylight.isDaytime && solarRad > 300) 
            results.phenomena.push('sun breaking through clouds');
    } else if (cloudCover < 90) {
        results.conditions.push('mostly cloudy');
        if (daylight.isDaytime) 
            results.phenomena.push('limited sunshine');
    } else {
        results.conditions.push('overcast');
        if (daylight.isDaytime) 
            results.phenomena.push('no direct sunshine');
    }

    const change6h = cloudCover6hAgo !== undefined ? cloudCover - cloudCover6hAgo : 0;
    if (change6h > 50) {
    ...
    if (cloudChanges.length > 10) {
        ...
        if (store.clouds.variability > 30) {
            ...
            if (month >= 4 && month <= 9) 
                results.phenomena.push('convective cloud development');
        } else if (store.clouds.variability < 10) 
    ...
    if (month >= 11 || month <= 2) { // Winter
        if (cloudCover > 80) {
            ...
            if (temp > 0) 
                results.phenomena.push('mild air mass');
    ...
    if (daylight.isDaytime && store.clouds.sunshineHours > 0) {
        ...
        if (sunshinePercentage > 80) 
            results.phenomena.push('abundant sunshine today');
         else if (sunshinePercentage < 20) {
            results.phenomena.push('minimal sunshine today');
            if (month >= 11 || month <= 1) 
                results.phenomena.push('typical winter gloom');
        }
    }

    if (!daylight.isDaytime) {
        if (cloudCover < 20) {
            const moonPhase = getMoonPhase(situation.date);
            if (moonPhase < 0.2 || moonPhase > 0.8) 
                results.phenomena.push('dark skies - good for astronomy');
        } else if (cloudCover > 80 && situation.location.lightPollution !== 'low') 
    ...
    if (store.clouds.overcastHours > 48) {
        ...
        if (month >= 11 || month <= 2) 
            results.phenomena.push('persistent winter gloom');
    } else if (store.clouds.clearHours > 48) 
        results.phenomena.push(`extended clear period: ${Math.round(store.clouds.clearHours / 24)} days`);
}
...
function interpretRainRate(results, situation, data, data_history, store) {
    const { timestamp, rainRate, temp, humidity, pressure, windSpeed, snowDepth } = data;
    const { month, hour, location } = situation;
    ...
    const oneHourAgo = timestamp - 60 * 60 * 1000,
     twentyFourHoursAgo = timestamp - 24 * 60 * 60 * 1000,
     sevenDaysAgo = timestamp - 7 * 24 * 60 * 60 * 1000;
    let accumulation1h = 0, accumulation24h = 0, accumulation7d = 0;
    ...
    Object.entries(data_history)
        ...
        .forEach(([ts, entry]) => {
            ...
            const timeDiff = (tsNum - (parseInt(ts) - 3600000)) / 3600000; // hours
            const accumulation = entry.rainRate * timeDiff;
            if (tsNum > oneHourAgo) 
                accumulation1h += accumulation;
            if (tsNum > twentyFourHoursAgo) {
                accumulation24h += accumulation;
                maxRate24h = Math.max(maxRate24h, entry.rainRate);
            }
            if (tsNum > sevenDaysAgo) 
                accumulation7d += accumulation;
            if (entry.rainRate > 0) {
                consecutiveWetHours++;
    ...
    if (rainRate > 0) {
        if (rainRate < 0.5) {
            ...
            if (temp < 0) 
                results.conditions.push('light snow likely');
             else if (temp < 3) 
                results.phenomena.push('mixed precipitation possible');
        } else if (rainRate < 2.5) 
            results.conditions.push('light to moderate rain');
         else if (rainRate < 4) 
            ...
            if (accumulation1h > 25) 
                results.alerts.push('flash flood risk');
        }
        if (store.precipitation.currentEvent) {
        ...
        if (store.precipitation.currentEvent && store.precipitation.currentEvent.accumulation > 1) {
            ...
            store.precipitation.currentEvent = null;
        }
        if (humidity > 90 && pressure < 1010) 
            results.phenomena.push('rain likely soon');
    }
    ...
    if (accumulation24h > 50) {
        ...
        if (accumulation24h > 100) 
            results.alerts.push('extreme precipitation event');
    } else if (accumulation24h > 25) 
        results.phenomena.push(`notable rainfall: ${accumulation24h.toFixed(1)}mm in 24 hours`);

    if (accumulation7d > 100) {
        results.phenomena.push(`wet week: ${accumulation7d.toFixed(0)}mm total`);
        if (location.forestCoverage === 'high') 
            results.phenomena.push('saturated forest floor');
    } else if (accumulation7d < 5) {
        results.phenomena.push('dry week');
        if (month >= 6 && month <= 8) {
            store.precipitation.consecutiveDryDays++;
            if (store.precipitation.consecutiveDryDays > 14) 
                results.alerts.push('drought conditions developing');
        }
    }
    ...
    if (month >= 12 || month <= 2) { // Winter
        if (rainRate > 0 && temp > 2) {
            ...
            if (snowDepth > 50) 
                results.alerts.push('rain on snow - rapid melt risk');
        }
    } else if (month >= 6 && month <= 8) { // Summer
    ...
    if (location.elevation < 200 && windSpeed > 5) {
        ...
        if (windDir !== undefined && (windDir > 225 && windDir < 315)) { // Westerly winds
            if (rainRate < 0.5 && humidity > 70) 
                results.phenomena.push('rain shadow effect possible');
        }
    }
    ...
    if (location.forestCoverage === 'high' && rainRate > 0) {
        if (rainRate < 0.5) 
         ...
         else if (consecutiveWetHours > 3) 
            results.phenomena.push('canopy drip enhancing ground moisture');
    }

    if (rainRate === 0 && accumulation24h < 0.5) 
        store.precipitation.consecutiveDryDays++;
     else 
        store.precipitation.consecutiveDryDays = 0;
}
...
function interpretComplexPhenomena(results, situation, data, data_history, store) {
    const { temp, humidity, pressure, windSpeed, rainRate, snowDepth, cloudCover } = data;
    const { month, hour, location, dewPoint, windChill, heatIndex } = situation;
    ...
    if (temp !== undefined && humidity !== undefined) {
        ...
        if (temp < 0 && humidity > 70 && rainRate > 0) {
            if (temp < -10) {
                ...
                results.phenomena.push('dry snow likely');
             else 
                results.phenomena.push('wet snow likely');
        }
        ...
        if ((temp < 0 || snowDepth > 0) && cloudCover > 70 && month >= 10 && month <= 3) {
            ...
            if (windSpeed > 5) {
                ...
                if (windSpeed > 10 && snowDepth > 100) 
                    results.alerts.push('significant tree damage risk');
            }
        }
        ...
        if (temp < 2 && temp > -8 && rainRate > 0) {
            if (temp > -2 && temp < 1 && humidity > 90) {
                ...
                if (location.forestCoverage === 'high') 
                    results.alerts.push('forest ice damage risk');
            } else {
        ...
        if (temp > 20 && humidity > 75) {
            ...
            if (temp > 25 && humidity > 80) 
                results.phenomena.push('unusually humid conditions');
        }
        ...
        if (month >= 5 && month <= 8 && temp > 22 && humidity < 40 && rainRate === 0) {
            ...
            const threeDaysAgo = data.timestamp - 3 * 24 * 60 * 60 * 1000;
            ...
            } else if (consecutiveDryHours > 48) 
    ...
    if (pressure !== undefined && humidity !== undefined) {
        ...
        if (pressure < 1000 && humidity > 75) {
            ...
            if (temp < -5 && month >= 11 && month <= 2) 
                results.phenomena.push('arctic high pressure - very cold');
        } else if (pressure > 1015 && pressure < 1020 && humidity > 60 && humidity < 75) 
            results.phenomena.push('fair weather');
    }
    ...
    if (temp !== undefined && windSpeed !== undefined && temp < 10 && windSpeed > 3) {
        ...
        if (windChillDiff >= 3) {
            ...
            if (windChill < -25) 
                results.alerts.push('extreme wind chill - frostbite risk in minutes');
             else if (windChill < -15) 
                results.alerts.push('severe wind chill - limit outdoor exposure');
             else if (windChill < -10 && windChillDiff >= 5) 
    ...
    if (temp !== undefined && humidity !== undefined && temp > 20 && humidity > 60) {
        ...
        if (heatIndexDiff >= 3) {
            ...
            if (heatIndex > 32) 
                results.alerts.push('heat stress warning - unusual for Nordic climate');
             else if (heatIndex > 28) 
                results.phenomena.push('uncomfortable heat and humidity combination');
        }
    }
    ...
    if (month >= 6 && month <= 8 && temp > 20 && humidity > 70 && pressure < 1010) {
        if (store.temperature && store.temperature.rateOfChange > 2) {
            ...
            if (windSpeed < 5 && hour >= 12 && hour <= 18) 
                results.phenomena.push('afternoon thunderstorms likely');
        }
    }
    ...
    if (temp < 2 && temp > -2 && humidity > 80) {
        if ((rainRate > 0 && temp < 0) || (store.temperature && store.temperature.extremes24h.min < 0 && temp > 0)) {
            ...
            if (location.forestCoverage === 'high') 
                results.alerts.push('treacherous conditions on forest trails');
        }
    }

    // Radiation fog potential (clear nights)
    if (cloudCover !== undefined && cloudCover < 20 && windSpeed < 2 && humidity > 80) {
        if (!situation.daylight.isDaytime && temp < dewPoint + 3) {
            results.phenomena.push('radiation fog likely overnight');
            if (location.forestCoverage === 'high') 
                results.phenomena.push('dense fog in forest clearings');
        }
    }
}
