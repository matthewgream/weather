...
class Formatter {
    ...
    static secondsToString(secs, options = {}) {
        return Formatter._valueToString(secs, (v) => Formatter._secondsToString(v, options));
    }
    static millisToString(millis, options = {}) {
        return Formatter._valueToString(millis, (v) => Formatter._secondsToString(v / 1000, options));
    }
    static timeToString(time, options = {}) {
        return Formatter._valueToString(time, (v) => (options.hoursOnly ? `${String(new Date(v).getHours()).padStart(2, '0')}:${String(new Date(v).getMinutes()).padStart(2, '0')}` : new Date(v).toISOString()));
    }
    ...
    static millimetresToString(millis, options = {}) {
        return Formatter._valueToString(millis, (v) => v.toFixed(1) + (options.noUnits ? '' : 'mm'));
    }
    static temperatureToString(temp, options = {}) {
        return Formatter._valueToString(temp, (v) => v.toFixed(1) + (options.noUnits ? '' : '°C'));
    }
    static pressureToString(pressure, options = {}) {
        return Formatter._valueToString(pressure, (v) => v.toFixed(1) + (options.noUnits ? '' : 'hPa'));
    }
    static humidityToString(humidity, options = {}) {
        return Formatter._valueToString(humidity, (v) => v.toFixed(1) + (options.noUnits ? '' : '%'));
    }
    static rainfallToString(rainfall, options = {}) {
        return Formatter._valueToString(rainfall, (v) => v.toFixed(1) + (options.noUnits ? '' : 'mm'));
    }
    static windspeedToString(windspeed, options = {}) {
        return Formatter._valueToString(windspeed, (v) => v.toFixed(1) + (options.noUnits ? '' : 'm/s'));
    }
    static snowdepthToString(snowdepth, options = {}) {
        return Formatter._valueToString(snowdepth, (v) => v.toFixed(1) + (options.noUnits ? '' : 'mm'));
    }
    static icedepthToString(icedepth, options = {}) {
        return Formatter._valueToString(icedepth, (v) => v.toFixed(1) + (options.noUnits ? '' : 'mm'));
    }
    static radiationToString(radiation, options = {}) {
        return Formatter._valueToString(radiation, (v) => v.toFixed(1) + (options.noUnits ? '' : 'µSv'));
    }
    static litresToString(litres, options = {}) {
        return Formatter._valueToString(litres, (v) => v.toFixed(1) + (options.noUnits ? '' : 'L'));
    }
    static solarToString(solar, options = {}) {
        return Formatter._valueToString(solar, (v) => v.toFixed(1) + (options.noUnits ? '' : 'W/m²'));
    }
    static uviToString(uvi) {
        return Formatter._valueToString(uvi, (v) => v.toFixed(0));
    }
    static bytesToString(bytes, options = {}) {
        return Formatter._valueToString(bytes, (v) => {
    ...
    static kpToString(kp, options = {}) {
        return Formatter._valueToString(kp, (v) => v.toFixed(1) + (options.noUnits ? '' : ' Kp'));
    }
    static distanceKmToString(distance, options = {}) {
        return Formatter._valueToString(distance, (v) => Math.round(v) + (options.noUnits ? '' : ' km/s'));
    }
    static densityToString(density, options = {}) {
        return Formatter._valueToString(density, (v) => v.toFixed(1) + (options.noUnits ? '' : ' p/cm³'));
    }
    static magneticFieldToString(nT, options = {}) {
        return Formatter._valueToString(nT, (v) => v.toFixed(1) + (options.noUnits ? '' : ' nT'));
    }
    static probabilityToString(prob, options = {}) {
        return Formatter._valueToString(prob, (v) => Math.round(v) + (options.noUnits ? '' : '%'));
    }
    static degreesToString(deg, options = {}) {
        return Formatter._valueToString(deg, (v) => (options.digits ? v.toFixed(options.digits) : Math.round(v)) + (options.noUnits ? '' : '°'));
    }
    static magnitudeToString(mag, options = {}) {
        return Formatter._valueToString(mag, (v) => (options.noUnits ? '' : 'mag ') + v.toFixed(1));
    }
    static energyJoulesE10ToString(energy, options = {}) {
        return Formatter._valueToString(energy, (v) => v.toFixed(1) + (options.noUnits ? '' : '×10¹⁰ J'));
    }
    static zhrToString(zhr, options = {}) {
        return Formatter._valueToString(zhr, (v) => '~' + Math.round(v) + (options.noUnits ? '' : ' ZHR'));
    }
    static hoursAgoToString(hours) {
        return Formatter._valueToString(hours, (v) => Math.round(v) + 'h ago');
    }
    static countToString(n) {
        return Formatter._valueToString(n, (v) => Math.round(v).toString());
    }
    ...
    static positionToString(altitude_, bearing_, direction_) {
        // XXX fix me
        return `${Formatter.altitudeToString(altitude_)} above horizon (bearing ${Math.round(bearing_)}°, ${direction_})`;
    }
    ...
    static timestampBracket(timestamp, now, timezone = 'UTC') {
        const formatted = Formatter.relativeAbsoluteTime(timestamp, now, timezone);
        return formatted === '-' ? '' : `[${formatted}]`;
    }
    static percentageToString(percent, options = {}) {
        return Formatter._valueToString(percent, (v) => Math.round(v) + (options.noUnits ? '' : '%'));
    }
