...
void mqtt_init_reconnect(struct mqtt_client *client,
                         ...
                         void (*publish_response_callback)(void** state, struct mqtt_response_publish *publish))
{
    client->handle = (mqtt_pal_handle) -1;
...
ssize_t __mqtt_send(struct mqtt_client *client) 
{
    ...
    for(; i < len; ++i) {
        ...
        {
          ssize_t tmp = mqtt_pal_sendall(client->handle, msg->start + client->send_offset, msg->size - client->send_offset, 0);
          if (tmp < 0) {
            client->error = (enum MQTTErrors)tmp;
            return tmp;
          } else {
...
ssize_t __mqtt_recv(struct mqtt_client *client)
{
    ...
    while(mqtt_recv_ret == MQTT_OK) {
        ...
        rv = mqtt_pal_recvall(client->handle, client->recv_buffer.curr, client->recv_buffer.curr_sz, 0);
        if (rv < 0) {
            /* an error occurred */
            client->error = (enum MQTTErrors)rv;
            return rv;
